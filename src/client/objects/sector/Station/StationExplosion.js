...
StationExplosion.prototype.start = function() {
  ...
  manager.shockwaveEmitter.explosion(station);
  ...
  manager.shockwaveEmitter.at({ center: station.position });

console.log('station position is ', station.position)
  // manager.shockwaveEmitter.explode(2);
  manager.shockwaveEmitter.explode(15);

  manager.glowEmitter.explosion(station);
  // manager.glowEmitter.at({ center: {x: station.position._x*4, y: station.position._y*4} });
  manager.glowEmitter.at({ center: station.position });
  manager.glowEmitter.explode(13);

  events.repeat(50, 100, function() {
    if(rnd.frac() > 0.35) {
      manager.explosionEmitter.explosion(station);
